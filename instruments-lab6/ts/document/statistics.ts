...
import {additionalFetchUsers, DataFetch} from "../data_fetch.js";
...
export function initStatisticsTable(queryBuilder: QueryBuilder, fetcher: DataFetch): void {
...
function initControls() {
    ...
    controls.querySelector<HTMLAnchorElement>('.next').onclick = (e) => {
        ...
        if (currentPage === maxPages || currentPage === maxPages - 1) {
            dataFetch.mockUsers(additionalFetchUsers).then(users => {
                tableQueryBuilder.append(users);
                updateTablePage();
                updateState();
            });
        }
